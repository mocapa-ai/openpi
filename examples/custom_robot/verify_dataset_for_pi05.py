...
def verify_dataset(dataset_name: str, visualize: bool = False):
    ...
    for pattern in important_files:
        ...
        if matches:
            ...
            if pattern == 'meta/stats.json':
                print(f"  ⚠️  {pattern} not found (will be computed during training)")
            else:
    ...
    if len(dataset) < 100:
        print(f"⚠️  WARNING: Only {len(dataset)} frames. Recommend at least 1000+ for training.")
    ...
    try:
        for ep_idx, episode in enumerate(dataset.episodes):
            ...
            if ep_length < 10:
                print(f"    ⚠️  Very short episode (< 10 frames)")
        
        if dataset.num_episodes > 10:
            print(f"  ... and {dataset.num_episodes - 10} more episodes")
    except Exception as e:
        print(f"  ⚠️  Could not read episode info: {e}")
    ...
    for feature in required_features:
        if feature not in dataset.features:
            ...
            print(f"✅ Has required feature: {feature}")
    
    if missing_required:
        print(f"\n❌ Missing required features: {missing_required}")
        return False
    ...
    if not camera_features:
        print(f"❌ No camera features found!")
        print(f"   Pi0.5 requires at least one camera view")
        return False
    
    print(f"\n✅ Found {len(camera_features)} camera feature(s): {camera_features}")
    ...
    try:
        ...
        if task_in_sample or task_in_features:
            print(f"✅ Has language instructions")
        else:
            print(f"⚠️  WARNING: No language instructions found. Pi0.5 requires task descriptions.")
        ...
        if 'state' in sample and 'actions' in sample:
            ...
            if state_shape != actions_shape:
                print(f"\n  ⚠️  WARNING: State and action dimensions differ:")
            ...
            if cam_name in sample:
                ...
                if len(image_np.shape) == 3:
                    ...
                    if image_np.shape[-1] != 3 and image_np.shape[-1] != 1:
                        print(f"    ⚠️  WARNING: Unexpected number of channels: {image_np.shape[-1]}")
                else:
                    print(f"    ⚠️  WARNING: Unexpected image dimensions: {image_np.shape}")
                ...
                if np.all(image_np == 0):
                    print(f"    ❌ All pixels are zero!")
                elif np.all(image_np == image_np.max()):
                    print(f"    ⚠️  All pixels are same value!")
                else:
                    print(f"    ✅ Image data looks valid")
        ...
        if 'task' in sample:
            ...
            print(f"\n  Task: '{task}'")
            if not task or task == "":
                print(f"    ⚠️  WARNING: Empty task description")
    ...
    try:
        ...
        if len(set(state_dims)) > 1:
            print(f"❌ Inconsistent state dimensions: {state_dims}")
            return False
        else:
            print(f"✅ Consistent state dimension: {state_dims[0]}")
        
        if len(set(action_dims)) > 1:
            print(f"❌ Inconsistent action dimensions: {action_dims}")
            return False
        else:
            print(f"✅ Consistent action dimension: {action_dims[0]}")
    ...
    for check_name, passed in checks:
        status = "✅" if passed else ("⚠️ " if "Sufficient" in check_name or "Multiple" in check_name else "❌")
    ...
    if all_critical_passed:
        print("✅ PASSED: Dataset is ready for Pi0.5 training!")
        print("\n📚 Next steps:")
        ...
        if not stats_path.exists():
            ...
            print(f"  1. Stats already computed ✅")
        ...
        print("❌ FAILED: Dataset has critical issues that need to be fixed")
        print("\n🔧 Please fix the issues marked with ❌ and re-run verification")
